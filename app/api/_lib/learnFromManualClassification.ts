...
export async function learnFromManualClassification(
  ...
  const { data: emp, error: fetchError } = await supabase
    .from("emprendedores")
    .select(
      "id, keywords_usuario_json, keywords_usuario, ai_keywords_json, ai_raw_classification_json"
    )
    .eq("id", emprendedorId)
    .single();
  ...
  const { data: pivotList } = await supabase
  ...
  const primaryPivot =
    Array.isArray(pivotList) && pivotList.length > 0 ? pivotList[0] : null;
  const oldSubcategoriaId = primaryPivot
    ? s((primaryPivot as { subcategoria_id?: unknown }).subcategoria_id) || null
    : null;

  let userTerms = readKeywordsUsuarioPreferJson(row);
  if (userTerms.length === 0) {
    const rawUser = row.keywords_usuario_json ?? row.keywords_usuario;
    if (rawUser && typeof rawUser === "object" && !Array.isArray(rawUser) && "keywords" in rawUser) {
      const arr = (rawUser as { keywords?: unknown[] }).keywords;
      if (Array.isArray(arr)) {
        userTerms = arr.map((x) => s(x)).filter(Boolean);
      }
    }
  }
  ...
  const clasificacionFinalJson = {
    primary_subcategoria_id: subcategoriaId,
    updated_at: now,
  };
