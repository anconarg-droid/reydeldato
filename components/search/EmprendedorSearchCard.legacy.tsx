...
import { getPlaceholderSinFotoSub, getPlaceholderSinFotoTitulo } from "@/lib/productRules";
...
export default function EmprendedorSearchCard(p: EmprendedorSearchCardProps) {
  ...
  const idleShadow = perfilCompleto
    ? "0 6px 22px rgba(5, 150, 105, 0.14), 0 2px 10px rgba(15, 23, 42, 0.05), 0 0 0 1px rgba(5, 150, 105, 0.08)"
    : "none";
  const hoverShadow = perfilCompleto
    ? "0 12px 32px rgba(5, 150, 105, 0.18), 0 4px 14px rgba(15, 23, 42, 0.07)"
  ...
  return (
    <article
      className={`flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl border bg-white ${
        perfilCompleto ? "border-[#059669]" : "border-slate-200"
      }`}
      style={{
    ...
    >
      ...
      >
        ...
        <div
          className={`relative h-40 w-full shrink-0 overflow-hidden rounded-xl sm:h-44 ${
            mostrarFoto
              ? "bg-slate-100"
              : "bg-slate-100 ring-1 ring-inset ring-slate-200/55"
          }`}
        >
          {mostrarFoto ? (
            <>
            ...
            <div className="flex h-full w-full flex-col items-center justify-center gap-1.5 px-3 pb-1 pt-0.5 text-center">
              <span
                className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/90 text-slate-500 shadow-sm ring-1 ring-slate-200/80"
                aria-hidden
              >
                <svg
                  viewBox="0 0 24 24"
              ...
              <span className="line-clamp-3 max-w-[13.5rem] text-[11px] font-medium leading-snug text-slate-500">
          ...
          {perfilCompleto ? (
            <div className="pointer-events-none absolute bottom-2 left-2">
              <span className="rounded-full bg-[#059669] px-3 py-1.5 text-[11px] font-bold tracking-wide text-white shadow-sm">
                Perfil completo
              </span>
            </div>
          ) : null}
