-- =============================================================================
-- Vista pública (frontend): vw_emprendedores_publico
-- =============================================================================
-- Objetivo:
-- - Evitar exponer columnas sensibles de `public.emprendedores` al usar anon key.
-- - Forzar al frontend/SSR público a consultar SIEMPRE esta vista.
--
-- NO incluye (intencional):
-- - email
-- - nombre_responsable / datos personales
...
SELECT
  ...
  -- Aliases contractuales (el código usa ambos nombres en distintos lugares)
  ...
  -- Contacto público (ojo: sin email; WhatsApp se usa para conversión)
  ...
  -- Locales físicos públicos (máx 3 en reglas de producto; orden: principal primero)
  (
    SELECT COALESCE(
      jsonb_agg(
        jsonb_build_object(
